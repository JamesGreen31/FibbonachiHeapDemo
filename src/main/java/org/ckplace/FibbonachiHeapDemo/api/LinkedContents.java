...
/**
 * An interface representing a collection of linked elements, each of which has a specified index in the collection.
 * The collection supports adding, removing, and accessing elements by their index.
 *
 * <p>Implementations of this interface should provide the following functionality:</p>
 * <ul>
 *   <li>{@link #get(int)} ()} - Return the Member at the given index.</li>
 *   <li>{@link #remove(int)} - Remove the Member at the given index.</li>
 *   <li>{@link #insert(int, Member)} - Insert the given Member at the given index.</li>
 *   <li>{@link #append(Member)} - Appebd the given Member to the list</li>
 * </ul>
 *
 * @param <T> the type of elements contained in the collection
 */
public interface LinkedContents<T> {
     ...
     * @throws IndexOutOfBoundsException if the index is out of range (index < 0 || index > size())
     */
    void insert(int index, Member<T> m);
